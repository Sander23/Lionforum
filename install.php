<?php
/***************************************************************************
* copyright            : (C) 2014 LIONware.
* email                : 
* $Id: install.php 5734 2014-8-6 16:10:12Z naudefj $
*
* This program is free software; you can redistribute it and/or modify it 
* under the terms of the BSD licenses
***************************************************************************/
...
function decompress_archive($data_root, $web_root)
{
	...
	/* Install from './lionforum_archive' file. */
	$fp = fopen('./lionforum_archive', 'rb');
	...
	do {
		...
		if (isset($meta_data[5])) {
			...
			if (defined('lion_debug')) echo "Extracting $path\n";
			$fp = @fopen($path, 'wb');
			if (!$fp) {
...
<head>
	<meta charset="utf-8">
	<title>lionforum Installation Wizard</title>
<?php
...
<table class="headtable"><tr>
  ...
  ?></td>
  <td><span class="linkhead">lionforum Installation Wizard</span></td>
  <td> &nbsp; </td>
</tr></table>
<br />
...
function lion_sql_error_handler($query, $error_string, $error_number, $server_version)
{
...
if (!count($_POST)) {
	...
	if (!version_compare(PHP_VERSION, '5.3.0', '>=')) {
		seterr('PHPVER', 'lionforum requires PHP 5.3.0 or later while you have version <?php echo PHP_VERSION; ?> Installed. Please rectify this and try again.');
	}
		...
		!$module_status['sqlsrv'] && !$module_status['pdo_sqlsrv'])
	{
		...
		seterr('NODB', 'lionforum requires a database to function. Your PHP installation does not support any of following databases: IBM DB2, Firebird, MySQL, Oracle, PosgreSQL, SQLite or MS-SQL Server. Please rectify this and try again.');
	}
	...
	if (!file_exists('./lionforum_archive')) {
...
if ($section == 'stor_path' || php_sapi_name() == 'cli') {
	if (php_sapi_name() == 'cli') {
		...
		while (!$_POST['SERVER_ROOT'] || !is_wr($_POST['SERVER_ROOT'])) {
			echo 'Path to lionforum web browseable files: ';
		...
		while (!$_POST['SERVER_DATA_ROOT'] || !is_wr($_POST['SERVER_DATA_ROOT'])) {
			echo 'Path to lionforum data files (non-browseable) ['. $_POST['SERVER_ROOT'] .']: ';
	...
	if (!$err) {
		...
		if (fud_ini_get('allow_url_fopen') && !empty($_POST['url_check'])) {
			...
			$fp = fopen($SERVER_ROOT .'lion_test_page.htm', 'wb');
			fwrite($fp, $check_time);
			fclose($fp);

			if (($d = @file_get_contents($WWW_ROOT .'lion_test_page.htm')) != $check_time) {
				seterr('WWW_ROOT', 'Your Forum URL and Web Directory do not appear to point to the same location on disk.<br /><small>Unable to load '. $SERVER_ROOT .'lion_test_page.htm as '. $WWW_ROOT .'lion_test_page.htm.<br />Error: '. $php_errormsg .'</small>');
			}
			unlink($SERVER_ROOT .'lion_test_page.htm');
		}
	}
	...
	if (!isset($GLOBALS['errors'])) {
		...
		$LION_OPT_1 = 1743713343;	// From default GLOBALS.php.
		if (!$module_status['pspell']) {
			$LION_OPT_1 ^= 2097152;	// Disable spell checker.
		}
		...
		change_global_settings(array(
			...
			'COOKIE_NAME'		=> 'lion_session_'. time(),
			'FUD_OPT_1'		=> $LION_OPT_1,
...
if ($section == 'db' || php_sapi_name() == 'cli') {
	...
	if (!isset($GLOBALS['errors'])) {
		drop_table('lion_forum_install_test_table', true);
		try {
			create_table('CREATE TABLE lion_forum_install_test_table (test_val INT)');
		} catch (Exception $e) {
			seterr('DBHOST_DBNAME', 'Please grant your database user permissions to create tables and try again.');
		}
		try {
			create_index('lion_forum_install_test_table', 'fud_forum_install_test_index', false, 'test_val');
		} catch (Exception $e) {
			seterr('DBHOST_DBNAME', 'Please grant your database user permissions to create indexes and try again.');
		}
		try {
			drop_table('lion_forum_install_test_table', false);
		} catch (Exception $e) {
...
if ($section == 'admin' || php_sapi_name() == 'cli') {
	...
	if(!isset($GLOBALS['errors'])) {
		...
		$uid = db_li('INSERT INTO '. $DBHOST_TBL_PREFIX .'users (login, alias, passwd, salt, name, email, users_opt, join_date, theme, time_zone) VALUES(\'Google\', \'Google\', \''. $passwd .'\', \''. $salt .'\', \'Googlebot\', \'Google@lion_spiders\', '. $bot_opts .', '. time() .', 1, \''. $SERVER_TZ .'\')', $ef, 1);
		q('INSERT INTO '. $DBHOST_TBL_PREFIX .'spiders (botname, useragent, theme, user_id, bot_opts) VALUES (\'Google\', \'Googlebot\', 1, '. $uid .', 1)');
		$uid = db_li('INSERT INTO '. $DBHOST_TBL_PREFIX .'users (login, alias, passwd, salt, name, email, users_opt, join_date, theme, time_zone) VALUES(\'Yahoo\', \'Yahoo\', \''. $passwd .'\', \''. $salt .'\', \'Yahoo!\', \'Yahoo@lion_spiders\', '. $bot_opts .', '. time() .', 1, \''. $SERVER_TZ .'\')', $ef, 1);
		q('INSERT INTO '. $DBHOST_TBL_PREFIX .'spiders (botname, useragent, theme, user_id, bot_opts) VALUES (\'Yahoo!\', \'Slurp\', 1, '. $uid .', 1)');
		$uid = db_li('INSERT INTO '. $DBHOST_TBL_PREFIX .'users (login, alias, passwd, salt, name, email, users_opt, join_date, theme, time_zone) VALUES(\'Bing\', \'Bing\', \''. $passwd .'\', \''. $salt .'\', \'Bing\', \'Bing@lion_spiders\', '. $bot_opts .', '. time() .', 1, \''. $SERVER_TZ .'\')', $ef, 1);
...
if (php_sapi_name() == 'cli') {
	...
	echo "\nCongratulations! Your Lionforum installation is now complete.\n";
...
switch ($section) {
	case 'welcome':
		...
		dialog_start('WELCOME TO LionFORUM!', '<p>Thanks for choosing Lionforum, the fastest, most secure, and most feature rich PHP based discussion forum available today.</p><p>This wizard will guide you through installing your forum. Kindly review your system information as set out below and click on <b>Start installer</b> if your system meets the minimum requirements. If you encounter any problems, please report them on the <a href="https://github.com/Sander23/Lionforum">support website</a>.</p><p><i>This program is free software; you can redistribute it and/or modify it under the terms of the <a href="http://opensource.org/licenses/BSD-3-Clause">BSD version v3</a> as published by the Free Software Foundation. This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.</i></p>');
		dialog_end($section);
	...
	if (!$module_status['mysqli']) {
		...
		prereq_row('MySQL:', 'Interface to the MySQL server, which is the recommended database for LIONforum.', 
